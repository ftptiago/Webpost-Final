...
$url = $_POST['url'];
    ...
    if ($titulo != NULL && $url != NULL) { 
        ...
        $insert = mysql_query("INSERT INTO `portfolio`(`id`, `url`, `titulo`, `image`)VALUES (NULL, '" .$url. "', '" .$titulo. "', '" .$pt_file."')",$con)or die(mysql_error());
        mysql_insert_id();
        
        if ($insert) {
            mysql_close($con); 
            echo "<script>if(window.confirm('Imagem cadastrada com sucesso!')) { window.location='admin.php';} else { window.close () } </script>";
        }
    } else {
