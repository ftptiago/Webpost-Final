...
$desc = $_POST['co_desc'];
    ...
    if ($nome != NULL && $link1 != NULL && $desc != NULL) { 
        ...
        $insert = mysql_query("UPDATE `confsite` SET `co_nome`= '$nome', `co_link1`= '$link1', `co_link2`= '$link2', `co_link3` = '$link3', `co_img`= '$pt_file', `co_desc`= '$desc' WHERE co_id = '$co_id'", $con)or die(mysql_error());
        mysql_insert_id();
